...
public class DailyRecordDaoHibernate extends GenericAccessDaoHibernate<DailyRecord, Long>
	...
	public List<DailyRecord> getDailyRecordsFrom(Date date, Long memberID) {
		String query = "SELECT dr FROM " + DailyRecord.class.getSimpleName() +"  dr WHERE dr.date>=:date AND dr.personId=:memberId";
	...
	public List<DailyRecord> getDailyRecords(Long memberID, Date after, Date before) {
		String query = "SELECT dr FROM " + DailyRecord.class.getSimpleName() +"  dr WHERE "
				+ "dr.date>=:after "
				+ "AND dr.date<=:before "
	...
	public List<DailyRecord> getNonEmptyDailyRecords(Long memberID, Date after,
			Date before) {
		String query = "SELECT dr FROM " + DailyRecord.class.getSimpleName() +"  dr, "
				+ Activity.class.getSimpleName()+" a WHERE "
				+ "dr.date>=:after "
				+ "AND dr.date<=:before "
				+ "AND dr.personId=:memberId "
				+ "AND a.dailyRecord=dr";
		Map<Object, Object> parameters = new HashMap<Object, Object>(3);
		parameters.put("after", after);
		parameters.put("before", before);
		parameters.put("memberId", memberID);
	...
	public DailyRecord getDailyRecordFrom(Date date, Long memberID) {
		String query = "SELECT dr FROM " + DailyRecord.class.getSimpleName() +"  dr WHERE DATE(dr.date)=DATE(?) AND dr.personId=?";
		Map<Object, Object> parameters = new HashMap<Object, Object>(2);
